...
class MLPencoder(nn.Module):
    ...
    MLP layer.
    """
    def __init__(self, dropout=0.1, act=nn.ReLU(),use_norm=True):
        super(MLPencoder, self).__init__()
        self.mlp1_1 = nn.Linear(256, 256)
        self.mlp1_2 = nn.Linear(256, 256)
        self.mlp1_3 = nn.Linear(256, 256)
        self.mlp1 = nn.Linear(256, 1)

    def forward(self, x):
        x = self.mlp1_1(x[0]) + self.mlp1_2(x[1]) + self.mlp1_3(x[2])
        x = self.mlp1(x)
        return x