...
class hyperGATConv(nn.Module):
    ...
    def forward(self, graph, feat, e_feat, res_attn=None):
        feat = self.manifold_in.logmap0(feat)
        res, attn = self.update(graph, feat, e_feat, res_attn)
        # print('res:', res)
        # print(self.activation)
        if self.activation:
            res = self.activation(res)
        res = self.manifold_out.expmap0(res)
        return res, attn
    
    def update(self, graph, feat, e_feat, res_attn=None):
        # print(graph)
        with graph.local_scope():
            if not self._allow_zero_in_degree:
                if (graph.in_degrees() == 0).any():
